...
tasks.jacocoTestCoverageVerification {
    dependsOn(tasks.test)
    // The copy package is prose. Constructing a verdict marks its whole reason string as covered
    // with no assertion about what it says, so leaving it in inflates the line ratio of every
    // package that has logic in it. The copy has its own tests, about its voice rather than its
    // coverage.
    classDirectories.setFrom(
        files(
            classDirectories.files.map { directory ->
                fileTree(directory) { exclude("**/copy/**") }
            },
        ),
    )
    violationRules {
