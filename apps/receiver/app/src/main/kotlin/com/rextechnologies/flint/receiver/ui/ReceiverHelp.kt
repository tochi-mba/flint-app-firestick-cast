...
internal fun ReceiverHelp(modifier: Modifier = Modifier) {
    ...
    if (progress.open) {
        ...
        Dialog(
            onDismissRequest = { progress = progress.hide() },
            properties = DialogProperties(usePlatformDefaultWidth = false, dismissOnClickOutside = false),
        ) {
            LaunchedEffect(Unit) { closeFocus.requestFocus() }
            Column(
                Modifier.fillMaxWidth(ReceiverOverscan.CONTENT_FRACTION)
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                    ...
                    }, modifier = Modifier.focusRequester(closeFocus)) { Text("Close") }
                    Button(onClick = { progress = progress.back() }, enabled = progress.index > 0) { Text("Previous") }
                    Button(onClick = {
                        if (progress.index == BrowserHelpTopics.entries.lastIndex) {
                            complete()
                        } else {
