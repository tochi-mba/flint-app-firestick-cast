...
object PhoneProbes {
     ...
     */
    fun probeVirtualDisplay(activity: Activity): ProbeOutcome = try {
    ...
    private fun runVirtualDisplayProbe(activity: Activity): ProbeOutcome {
        ...
        try {
            ...
            presentation = showProbeFrame(activity, display.display) ?: return ProbeOutcome.UNSUPPORTED
            return if (probeFrameArrived(reader)) ProbeOutcome.SUPPORTED else ProbeOutcome.UNSUPPORTED
        } finally {
    ...
    /**
     * Waits briefly for the display to produce a frame and checks that it is the colour that was
     * painted.
     *
     ...
     */
    private fun probeFrameArrived(reader: ImageReader): Boolean {
        ...
        while (System.nanoTime() < deadline) {
            ...
            try {
                ...
                val red = buffer.get(0).toInt() and 0xff
                val green = buffer.get(1).toInt() and 0xff
                val blue = buffer.get(2).toInt() and 0xff
                return red == PROBE_RED && green == PROBE_GREEN && blue == PROBE_BLUE
            } finally {
                image.close()
            }
        }
    ...
    private const val PROBE_TIMEOUT_NANOS = 2_000_000_000L
    private const val PROBE_POLL_MILLIS = 16L
